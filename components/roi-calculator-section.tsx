...
export function ROICalculatorSection() {
  ...
  const getVenueDefaults = () => {
    ...
    return venueDefaults[inputs.venueType as keyof typeof venueDefaults] || venueDefaults.default
  }

  useEffect(() => {
    const defaults = getVenueDefaults()
    setInputs((prev) => ({ ...prev, averageIncidentCost: defaults.incidentCost }))
  }, [inputs.venueType])
  ...
  const currentAnnualIncidents = Math.round((inputs.venueCapacity / 5000) * inputs.eventsPerMonth * 0.3) // Rough incident rate
  const currentAnnualCost = currentAnnualIncidents * inputs.averageIncidentCost

  // Improved metrics with VenueShield
  const preventedIncidents = Math.round(currentAnnualIncidents * (improvements.incidentReduction / 100))
  ...
  const insuranceSavings = Math.round(currentAnnualCost * 0.15 * (improvements.insuranceSavings / 100))
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto text-balance">
            Calculate how much VenueShield AI could save your venue through incident prevention and reduced liability
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-stretch">
          ...
          >
            <Card className="p-6 md:p-8 bg-[radial-gradient(35%_128px_at_50%_0%,theme(backgroundColor.white/15%),theme(backgroundColor.white/5%))] border-white/20 backdrop-blur-sm shadow-2xl h-full flex flex-col">
              <h3 className="text-xl md:text-2xl font-semibold text-white mb-6 md:mb-8">Your Venue Metrics</h3>
              ...
              <div className="space-y-8 flex-1">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setInputs((prev) => ({ ...prev, venueType: value }))}
                  >
                    <SelectTrigger className="bg-gray-700/50 border-gray-600 text-white">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-gray-800 border-gray-700">
                      <SelectItem value="arena">Arena / Sports Venue</SelectItem>
                      ...
                      <SelectItem value="nightclub">Nightclub / Nightlife</SelectItem>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-3">
                    Venue Capacity:{" "}
                    <span className="text-white font-semibold">{inputs.venueCapacity.toLocaleString()}</span>
                  </label>
                  <Slider
                    value={[inputs.venueCapacity]}
                    onValueChange={([value]) => setInputs((prev) => ({ ...prev, venueCapacity: value }))}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-3">
                    Events Per Month: <span className="text-white font-semibold">{inputs.eventsPerMonth}</span>
                  </label>
                  <Slider
                    value={[inputs.eventsPerMonth]}
                    onValueChange={([value]) => setInputs((prev) => ({ ...prev, eventsPerMonth: value }))}
                    max={30}
                    min={1}
                    step={1}
                    className="w-full"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-3">
                    Average Incident Cost:{" "}
                    <span className="text-white font-semibold">${inputs.averageIncidentCost.toLocaleString()}</span>
                  </label>
                  <Slider
                    value={[inputs.averageIncidentCost]}
                    onValueChange={([value]) => setInputs((prev) => ({ ...prev, averageIncidentCost: value }))}
                    max={100000}
                    min={5000}
                    step={5000}
                    className="w-full"
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-white/5">
                      ...
                      <div>
                        <p className="text-sm text-gray-300">
                          <span className="font-medium text-white">Incident reduction:</span> Venues see{" "}
                          {venueConfig.incidentReduction}% fewer safety incidents with AI monitoring
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-white/5">
                      ...
                      <div>
                        <p className="text-sm text-gray-300">
                          <span className="font-medium text-white">Response time:</span> AI detection is{" "}
                          {venueConfig.responseImprovement}% faster than manual monitoring
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-white/5">
                      ...
                      <div>
                        <p className="text-sm text-gray-300">
                          <span className="font-medium text-white">Insurance impact:</span> Average{" "}
                          {venueConfig.insuranceSavings}% reduction in premiums with VenueShield
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-6 flex-1">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="text-center p-3 md:p-4 rounded-lg bg-gray-700/30">
                    <div className="text-xs md:text-sm text-gray-400 mb-1">Current Annual Incidents</div>
                    <div className="text-xl md:text-2xl font-bold text-white">{currentAnnualIncidents}</div>
                  ...
                  <div className="text-center p-3 md:p-4 rounded-lg bg-white/10 border border-white/20">
                    <div className="text-xs md:text-sm text-gray-300 mb-1">With VenueShield</div>
                    <div className="text-xl md:text-2xl font-bold text-white">{reducedIncidents}</div>
                ...
                <div className="space-y-3 md:space-y-4">
                  <div className="flex items-center justify-between p-3 md:p-4 rounded-lg bg-white/5 border border-white/10">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm md:text-base text-white">Incidents Prevented</span>
                    </div>
                    <span className="text-lg md:text-xl font-bold text-white">{preventedIncidents}</span>
                  </div>

                  <div className="flex items-center justify-between p-3 md:p-4 rounded-lg bg-white/5 border border-white/10">
                    <div className="flex items-center gap-3">
                      <DollarSign className="w-4 h-4 md:w-5 md:h-5 text-gray-300" />
                      <span className="text-sm md:text-base text-white">Incident Cost Savings</span>
                    </div>
                    <span className="text-lg md:text-xl font-bold text-white">${annualSavings.toLocaleString()}</span>
                  </div>

                  <div className="flex items-center justify-between p-3 md:p-4 rounded-lg bg-white/5 border border-white/10">
                    <div className="flex items-center gap-3">
                      <TrendingUp className="w-4 h-4 md:w-5 md:h-5 text-gray-300" />
                      <span className="text-sm md:text-base text-white">Insurance Premium Savings</span>
                    </div>
                ...
                <div className="mt-6 md:mt-8 p-4 md:p-6 rounded-lg bg-white/5 border border-white/10">
                  <div className="text-center">
                    <div className="text-xs md:text-sm text-gray-300 mb-2">Estimated Annual Savings</div>
        ...
        >
          <p className="text-sm text-gray-400 mt-4">* Results based on industry averages and may vary by venue</p>
        </div>
      </div>
    </section>
  )
